...
public class SimulationRunnerImpl implements SimulationRunner {
    ...
    public void run(SimulationData simulationData) {
        ...
        for (var i = 0; i < nbOfActions; i++) {
            for (var explorer : simulationData.getExplorers()) {
               act(simulationData.getTreasureMap(), explorer);
               explorer.getActionSequence().removeFirst();
            }
        }
    }

    void act(List<List<MapCell>> treasureMap, Explorer explorer) {
        switch (explorer.getActionSequence().getFirst()) {
            case "A" -> moveToDirection(treasureMap, explorer);
            case "G" -> turn(explorer, true);
            case "D" -> turn(explorer, false);
        }
    }

    void moveToDirection(List<List<MapCell>> treasureMap, Explorer explorer) {
        switch(explorer.getDirection()) {
            case ExplorerDirection.NORTH -> move(treasureMap, explorer, explorer.getCoordX(), explorer.getCoordY() - 1);
            case ExplorerDirection.SOUTH -> move(treasureMap, explorer, explorer.getCoordX(), explorer.getCoordY() + 1);
            case ExplorerDirection.EAST -> move(treasureMap, explorer, explorer.getCoordX() + 1, explorer.getCoordY());
            case ExplorerDirection.WEST -> move(treasureMap, explorer, explorer.getCoordX() - 1, explorer.getCoordY());
        }
    }

    void move(List<List<MapCell>> treasureMap, Explorer explorer, int newX, int newY) {
        if (canMoveTo(treasureMap, newX, newY)) {
    ...
    boolean canMoveTo(List<List<MapCell>> treasureMap, int newX, int newY) {
